...
- Khi doi rule nghiep vu (do dai ma, bat buoc field), cap nhat tai day de trang/components khong bi roi.
*/

import type { TrayPayload } from '../types/trayTypes'

export function validateTrayForm(form: TrayPayload): string | null {
  // Ghi chú: Backend moi chi yeu cau product_id + location_id; tray_code/qr_code duoc auto-gen.
...
export function normalizeTrayPayload(form: TrayPayload): TrayPayload {
  // Ghi chú: Trim text va ep number de payload dong nhat truoc khi goi API.
  return {
