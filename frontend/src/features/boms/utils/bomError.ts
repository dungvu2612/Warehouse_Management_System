...
export function mapBOMApiError(error: unknown): string {
  ...
  if (code === 'BOM_PARENT_PRODUCT_NOT_FOUND') return message || 'Không tìm thấy thành phẩm cha hoặc thành phẩm đã bị khóa.'
  if (code === 'BOM_COMPONENT_PRODUCTS_NOT_FOUND') return message || 'Một hoặc nhiều linh kiện không tồn tại hoặc đã bị khóa.'
